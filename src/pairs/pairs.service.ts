...
export class PairsService {
    ...
    for (const pair of pairs) {
      ...
      for (let i = 1; i <= 365; i++) {
        ...
        if (i === 1) {
          ...
          ratioChange = exactMath.add(
            avgChange,
            exactMath.mul(avgChangeDiff, i - 1),
          );
        } else if (i <= 20) {
          ratioChange = exactMath.mul(CONFIG_HEDGE_DIFF.D16_D20, avgChange);
        } else if (i <= 27) {
          ratioChange = exactMath.mul(CONFIG_HEDGE_DIFF.D21_D27, avgChange);
        } else if (i <= 30) {
          ratioChange = exactMath.mul(CONFIG_HEDGE_DIFF.D28_D30, avgChange);
        } else if (i <= 365) {
          ratioChange = exactMath.mul(CONFIG_HEDGE_DIFF.D31_D365, avgChange);
        }
  ...
  async updateHourChangeAvgConfig(period: '12' | '4' | '1') {
    this.logger.log('UPDATE HOUR CHANGE TOKEN', period, new Date());
