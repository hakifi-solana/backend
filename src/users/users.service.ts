...
export class UsersService {
  ...
  async getFriendDetail(userId: string, friendId: string) {
    ...
    const result: any = await this.prismaService.commission.aggregateRaw({
      pipeline: [
        {
          $match: {
            toUserId: userId,
    ...
    const totalMargin = result[0]?.totalMargin ?? 0;
    const totalContract = result[0]?.totalContract ?? 0;
  ...
  async updateUserStatsUpdatedInsurance(insurance: Insurance) {
    ...
    switch (insurance.state) {
      case InsuranceState.CLAIMED:
        ...
        pnl = insurance.q_claim - insurance.margin;
        break;
