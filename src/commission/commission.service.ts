        ...
        if (shareBackAmount > 0) {
          try {
            await this.createCommision({
              ...
              commissionType: CommissionType.INDIRECT,
              type: 0,
  ...
  private getCommissionAmount(
    ...
    switch (insurance.state) {
      ...
      case InsuranceState.LIQUIDATED:
        ...
        if (isDirect) return LIQUIDATED_COMMISSION_DIRECT * insurance.margin;
        // 0.3% * Ký quỹ
        else return COMMISSION_INDIRECT_RATE * insurance.margin;
      ...
      case InsuranceState.CLAIMED:
        ...
        payback = insurance.margin + insurance.q_claim;
        // Trực tiếp = Config Tỷ lệ hh theo cấp * Giá trị Payback * Tỷ lệ chia sẻ (nếu có)
        if (isDirect)
          return PAYBACK_COMMISSION_DIRECT_BY_LEVEL[parent.level] * payback;
        // Gián tiếp = Config Tỷ lệ hh gián tiếp * Giá trị Payback
        else return COMMISSION_INDIRECT_RATE * payback;
      default:
        return 0;
    }
  }
